...
import org.pbccrc.zsls.utils.clock.Clock;
...
abstract public class Shell {
  ...
  private static final int ERR_MSG_BUFF_SIZE = 20 * 1024;
  /**
  ...
  protected void run() throws IOException {
    if (lastTime + interval > clock.getTime())
      return;
  ...
  public static class ExitCodeException extends IOException {
	private static final long serialVersionUID = -5770043785611714671L;
  ...
  public interface CommandExecutor {
    ...
    String getKeyMessage() throws IOException;
  ...
  public static class ShellCommandExecutor extends Shell 
    ...
    protected String runtimeMeta;
    
    public String getRuntimeMeta() {
		return runtimeMeta;
	}

	public void setRuntimeMeta(String runtimeMeta) {
	...
	@Override
	public String getKeyMessage() throws IOException {
	  return keyMessage;
	}
  }
  
  /**
   * parse keyword message from stdout
   */
  public static class KeyShellCommandExecutor extends ShellCommandExecutor {

	public KeyShellCommandExecutor(String[] execString) {
		super(execString);
	}
	public KeyShellCommandExecutor(String[] execString, File dir) {
		super(execString, dir);
	}
	public KeyShellCommandExecutor(String[] execString, File dir, Map<String, String> env) {
    ...
    protected void parseExecResult(BufferedReader lines) throws IOException {
      String line = null;
      String headBlock = ZslsConstants.TASK_KEY_MSG_PREF;
      String tailBlock = ZslsConstants.TASK_KEY_MSG_APPEND;
      String headBlock2 = ZslsConstants.TASK_RUNTIME_META_PREF;
      String tailBlock2 = ZslsConstants.TASK_RUNTIME_META_APPEND;
      while ((line = lines.readLine()) != null) {
        int idx = line.indexOf(headBlock);
        if (idx >= 0) {
          int end = line.indexOf(tailBlock, idx + headBlock.length());
          if (end > 0)
            keyMessage = line.substring(idx + headBlock.length(), end);
        }
        else {
          int idx2 = line.indexOf(headBlock2);
          if (idx2 >= 0) {
        	int end2 = line.indexOf(tailBlock2, idx2 + headBlock2.length());
        	  if (end2 > 0)
	            runtimeMeta = line.substring(idx2 + headBlock2.length(), end2);
          }
        }
      }
    }
