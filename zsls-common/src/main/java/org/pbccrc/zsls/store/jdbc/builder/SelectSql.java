...
public class SelectSql {
    ...
    public SelectSql inTransaction(Transaction trans) {
    	this.trans = trans;
    	return this;
    }
    ...
    public SelectSql between(String column, Object start, Object end) {
    ...
    public <T> T single(ResultSetHandler<T> handler) {
        try {
            return sqlTemplate.query(getSQL(), trans, handler, params.toArray());
        } catch (Exception e) {
            throw new JdbcException("Select SQL Error:" + getSQL(), e);
        }
    }
    ...
    public <T> SelectSql in(Collection<T> values) {
    	sql.append(" IN ");
